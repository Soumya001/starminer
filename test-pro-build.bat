@echo off
REM ============================================================================
REM Local PRO-build test harness.
REM
REM Configures, builds, and runs ctest with -DCOLLIDER_PRO=ON in a dedicated
REM build directory (build-pro). Mirrors the production Pro pipeline so a
REM green run here is the same shape as a green private CI run.
REM
REM Expected: 16 tests pass. test_gpu_hash160 is included because it links
...
REM   .\test-pro-build.bat clean      REM wipe build-pro first
REM ============================================================================

setlocal enabledelayedexpansion

set BUILD_DIR=build-pro
...
echo.
echo [*] Configuring (CUDA, NATIVE arch, PRO=ON)...
...
REM ----- Step 6: Test -------------------------------------------------------
echo.
echo [*] Running ctest (expect 16 tests including GpuHash160)...
ctest --test-dir "%REPO_ROOT%\%BUILD_DIR%" --output-on-failure
set CTEST_EXIT=%ERRORLEVEL%

echo.
if %CTEST_EXIT% EQU 0 (
    echo [+] PRO build: ALL TESTS PASSED.
) else (
    echo [!] PRO build: one or more tests FAILED.
)
exit /b %CTEST_EXIT%
