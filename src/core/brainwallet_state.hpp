...
struct BrainWalletSearchState {
    ...
    // Advanced iteration mode tracking (world-class infinite scanning)
    uint8_t iteration_mode = 0;           // Current IterationMode enum value
    size_t mode_sub_iteration = 0;        // Sub-iteration within current mode
    size_t stacked_rule_depth = 2;        // Rule stacking depth (2-3)
    size_t mask_length = 1;               // Current mask length for hybrid mode
    size_t combinator_word2_idx = 0;      // Second word index for combinator
    ...
    uint16_t state_version = 2;           // State format version (2 = with iteration modes)
...
public:
    ...
    static std::string get_timestamp() {
        // Track-f F-17: localtime_r/_s avoid the static-buffer race that
        // std::localtime has when called concurrently from Logger and main.
    ...
    static uint32_t compute_checksum(const BrainWalletSearchState& state) {
        ...
        // Include new iteration mode fields in checksum
        mix(state.iteration_mode);
        mix(state.mode_sub_iteration);
        mix(state.stacked_rule_depth);
        mix(state.mask_length);
        mix(state.combinator_word2_idx);
        mix(state.state_version);
    ...
    static bool save_state(const BrainWalletSearchState& state) {
        ...
        // Write to temporary file first
        {
            ...
            // Advanced iteration mode tracking (v2)
            file << "iteration_mode=" << static_cast<int>(state.iteration_mode) << "\n";
            file << "mode_sub_iteration=" << state.mode_sub_iteration << "\n";
            file << "stacked_rule_depth=" << state.stacked_rule_depth << "\n";
            file << "mask_length=" << state.mask_length << "\n";
            file << "combinator_word2_idx=" << state.combinator_word2_idx << "\n";
            file << "state_version=" << state.state_version << "\n";
    ...
    static BrainWalletSearchState load_state() {
        ...
        while (std::getline(file, line)) {
            ...
            try {
                ...
                // Advanced iteration mode fields (v2)
                } else if (key == "iteration_mode") {
                    state.iteration_mode = static_cast<uint8_t>(std::stoul(value));
                } else if (key == "mode_sub_iteration") {
                    state.mode_sub_iteration = std::stoull(value);
                } else if (key == "stacked_rule_depth") {
                    state.stacked_rule_depth = std::stoull(value);
                } else if (key == "mask_length") {
                    state.mask_length = std::stoull(value);
                } else if (key == "combinator_word2_idx") {
                    state.combinator_word2_idx = std::stoull(value);
                } else if (key == "state_version") {
                    state.state_version = static_cast<uint16_t>(std::stoul(value));
                // Metadata
