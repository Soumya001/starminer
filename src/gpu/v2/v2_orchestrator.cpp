...
uint64_t MultiAddressSession::total_dropped_matches() const { return 0; }

uint64_t MultiAddressSession::total_overflow_batches() const { return 0; }
...
int run_v2_orchestrator(const OrchestratorOptions& opts) {
    ...
    std::fprintf(stderr,
        "[v2] --puzzle-only-v2 requires a CUDA build (STARMINER_USE_CUDA=ON).\n");
    return 64;
}
